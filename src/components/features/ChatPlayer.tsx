...
type Shown = Step & { time: string; read: boolean };
...
export function ChatPlayer({ t }: { t: Translations }) {
  ...
  useEffect(() => {
    // reduced motion → render one full scenario, no looping
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduce) {
      // one-shot static render for reduced-motion users
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setShown(SCENARIOS[0].map((s, i) => ({ ...s, time: `14:3${1 + i}`, read: true })));
      return;
    }
    let cancelled = false;
    let visible = true;
    ...
    const sleep = (ms: number) =>
      new Promise<void>((res) => {
        const tick = () => (cancelled ? res() : visible ? setTimeout(res, ms) : setTimeout(tick, 200));
        tick();
      });
    ...
    (async () => {
      ...
      while (!cancelled) {
        ...
        for (let i = 0; i < scenario.length && !cancelled; i++) {
          ...
          setTyping(step.f);
          await sleep(step.img ? 900 : 1150);
          if (cancelled) break;
          setTyping(null);
          acc.push({ ...step, time: `14:3${(3 + i) % 10}`, read: false });
          setShown([...acc]);
          // a beat later, the previous driver message reads as seen
          await sleep(step.img ? 1300 : 1050);
          acc.forEach((m) => { if (m.f === "driver") m.read = true; });
          setShown([...acc]);
        }
        await sleep(2600);
      ...
      <div className="w-full h-full flex gap-[2.4cqw]">
        ...
        <div className="flex-1 rounded-[1.5cqw] bg-slate-900/70 border border-slate-700/70 flex flex-col overflow-hidden min-w-0">
          ...
          <div ref={dashRef} className="flex-1 flex flex-col gap-[1cqw] px-[1.8cqw] py-[1.4cqw] min-h-0 overflow-hidden">
            {shown.map((m, i) => {
              const own = m.f === "disp";
              return (
                <div key={i} className={`flex items-end gap-[1cqw] ${own ? "flex-row-reverse" : "flex-row"}`}>
                  {!own && (
                  ...
                  <div className={`max-w-[74%] rounded-[1.1cqw] px-[1.4cqw] py-[0.9cqw] ${own ? "bg-blue-600 rounded-br-[0.3cqw]" : "bg-slate-700 rounded-bl-[0.3cqw]"}`}>
                    {m.img ? (
                      ...
                      <span className="text-white text-[1.5cqw] leading-snug block">{txt(m.k)}</span>
                    )}
                    <span className={`flex items-center gap-[0.4cqw] mt-[0.3cqw] text-[1.1cqw] ${own ? "justify-end text-blue-200" : "text-slate-400"}`}>
                      {m.time}
                      {own && (
                        <svg viewBox="0 0 24 24" className={`w-[1.6cqw] h-[1.6cqw] ${m.read ? "text-sky-300" : "text-blue-300/70"}`} fill="none" stroke="currentColor" strokeWidth="3">
          ...
          <div className="flex items-center gap-[1cqw] px-[1.8cqw] py-[1.1cqw] border-t border-slate-700/70">
            ...
            <span className="w-[3cqw] h-[3cqw] rounded-full bg-blue-600 flex items-center justify-center text-white [&_svg]:w-[1.7cqw] [&_svg]:h-[1.7cqw]"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M3 20l18-8L3 4l4 8-4 8z" /></svg></span>
          </div>
        </div>
        ...
        <PhoneFrame16>
          ...
          <div className="shrink-0 bg-gradient-to-r from-blue-600 to-blue-700 flex items-center gap-[3cqw] px-[4cqw] pt-[6cqw] pb-[3cqw]">
            ...
            <span className="flex flex-col leading-tight min-w-0">
              <span className="text-white font-bold text-[4.4cqw]">{c.uiPushApp}</span>
              <span className="text-[3.4cqw] leading-none h-[3.8cqw] text-blue-100">{typing === "disp" ? `London Warehouse ${c.uiTyping}` : "London Warehouse"}</span>
            </span>
          </div>
          {/* thread */}
          <div ref={phoneRef} className="flex-1 flex flex-col gap-[2.4cqw] px-[3.5cqw] py-[3cqw] min-h-0 overflow-hidden bg-slate-950">
            {shown.map((m, i) => {
              const own = m.f === "driver";
              return (
                <div key={i} className={`flex items-end gap-[2cqw] ${own ? "flex-row-reverse" : "flex-row"}`}>
                  ...
                  <div className={`max-w-[80%] rounded-[4cqw] px-[3.4cqw] py-[2.6cqw] ${own ? "bg-blue-600 rounded-br-[1cqw]" : "bg-slate-700 rounded-bl-[1cqw]"}`}>
                    {m.img ? (
                      ...
                      <span className="text-white text-[4.4cqw] leading-snug block">{txt(m.k)}</span>
                    )}
                    <span className={`flex items-center gap-[1cqw] mt-[0.8cqw] text-[3cqw] ${own ? "justify-end text-blue-200" : "text-slate-400"}`}>
                      {m.time}
                      {own && (
                        <svg viewBox="0 0 24 24" className={`w-[3.6cqw] h-[3.6cqw] ${m.read ? "text-sky-300" : "text-blue-200/70"}`} fill="none" stroke="currentColor" strokeWidth="3">
                          <path d="M1 12l6 6L17 6M8 16l1.5 1.5L22 6" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      )}
                    </span>
